---
title: JavaScript 对象模型（三）：用一个例子，串起 ECMAScript 对象模型
created: 2026-06-15
updated: 2026-06-18T20:26:44
tags:
  - JavaScript
  - ECMAScript
  - Object
...
description: 从一条触发原型 setter 的普通赋值出发，深入解析 ECMAScript 的 Internal Method、Property Descriptor 与 Internal Slot 三层机制如何协作，理解 JavaScript 对象模型的完整结构。
status: evergreen
series:
...
N_L["OrdinaryGetPrototypeOf 从对象的 [[Prototype]] 内部槽取得其原型对象的引用"] -.-> L2
...
> **注：** 本图的重点不在于逐帧还原 ECMA-262 算法，而在于借 `obj.name = "张三"` 的执行路径，观察 ECMAScript 如何将对象的能力拆分为 Internal Method、Property Descriptor 和 Internal Slot 三个相互协作的层次。部分规范细节已做抽象和省略，目的是呈现对象模型的组织思想，而非算法的完整实现。
...
| Abstract Operation（抽象操作） | `OrdinarySet`、`OrdinarySetWithOwnDescriptor`、`OrdinaryGetOwnProperty`、`OrdinaryGetPrototypeOf`、`IsAccessorDescriptor`、`IsDataDescriptor`、`Call` |
| Property Descriptor（属性描述符） | `ownDesc`、`[[Set]]`|
| Internal Slot（内部槽位） | `[[Prototype]]` |

这里有一个细节值得注意：这些名字虽然都带着 `[[...]]`，却并不属于同一类东西。`[[...]]` 只是 ECMAScript 规范的一种命名记号，同样的记号可能指向三种完全不同的概念：

- **Internal Method**：定义对象的行为，例如 `[[Set]]`、`[[GetOwnProperty]]`、`[[Call]]`；
- **Internal Slot**：保存对象的状态，例如 `[[Prototype]]`、`[[Extensible]]`；
- **Property Descriptor Field**：描述 Property 的语义，例如 `[[Value]]`、`[[Writable]]`，以及访问器属性中的 `[[Get]]`、`[[Set]]`。

其中最容易混淆的是访问器描述符的 `[[Set]]` 和对象的 `[[Set]]` 内部方法：前者描述某个 Property 的 setter 是什么，后者描述对象如何响应一次赋值请求。后面遇到时还会再次提醒这一点。
...
- `OrdinaryGetOwnProperty` 产出 `ownDesc`，描述当前属性究竟是什么；
- `OrdinaryGetPrototypeOf` 读取 `[[Prototype]]`，决定下一步应该去哪个对象继续查找。

整条执行路径，就是这些角色彼此协作的过程。

因此，接下来的内容也不会按照 ECMAScript 规范逐个解释这些术语，而是沿着这条赋值路径，依次认识它们在对象模型中的职责。
...
- Proxy：`Proxy.[[Set]]` 先查找 `set` trap；若存在则调用 trap，否则转发到 `target.[[Set]]`

- TypedArray：`TypedArray.[[Get]]`、`TypedArray.[[Set]]` 都拥有特殊实现

需要注意的是，Exotic Object 并不意味着所有 Internal Method 都被重写。Array 并未重写 `[[Set]]`，赋值仍走 `OrdinarySet`；但 `OrdinarySet` 执行过程中最终会触发 `Receiver.[[DefineOwnProperty]]`，而 Array 重写了 `[[DefineOwnProperty]]` 以实现 `length` 的自动维护。因此，`arr[100] = 1` 看似简单，底层仍会触发 Array 特有的长度调整逻辑。
...
需要注意的是，这里的 `Call` 与 `[[Set]]` 并不是对称关系。

- `[[Set]]` 是 **Internal Method**，属于对象的协议接口——语言只规定「对象必须支持设置属性」，至于如何响应由对象自行决定；

- `Call` 是 **Abstract Operation**，更接近一个工具函数：它接收一个函数对象 `F`，负责调用前的一些公共处理，最终委托给 `F.[[Call]]` 完成实际执行。

也就是说，`Call` 并不是函数调用独有的「协议入口」，而是规范在 `EvaluateCall` 求值之后、`[[Call]]` 之前安排的一段公共逻辑。所以不能简单理解成「赋值用 Internal Method 做入口、函数调用用 Abstract Operation 做入口」的对称设计。（与赋值表达式类似，函数调用 `fn()` 实际会先经过 `EvaluateCall` 等表达式求值步骤，再到 `Call`；本文同样省略了求值阶段的处理。）

真正值得记住的是：两者都呈现出「入口 → 实现」的两层结构，只是这次入口恰好落在 Abstract Operation（`Call`）这一层，再由具体函数对象实现 `[[Call]]`。

但这只是函数调用这一处的设计选择，**并不意味着规范存在「入口层级可以随意翻转」的设计规律**。ECMAScript 并没有规定哪一层必须负责分发、哪一层必须负责实现，规范会根据每处操作的实际情况安排层级。因此，在阅读规范时，不应把 `Internal Method → Abstract Operation` 或 `Abstract Operation → Internal Method` 当成固定规则套用。
...
[[GetOwnProperty]]
↓
ownDesc = OrdinaryGetOwnProperty(O, P)
```
...
因此，Property Descriptor 可以理解为属性元信息的统一描述，供各种内部算法读取和决策。规范语境下它本质是一种 Specification Record，工程上常称之为元信息（Metadata）。算法并不直接研究属性，而是研究它的 Descriptor。（注：部分 Exotic Object 的属性语义还涉及 `[[Get]]`、`[[Set]]` 等 Internal Method 的特殊实现，Descriptor 本身无法覆盖全部语义。）
...
它们不是对象对外提供的接口，而是对象语义本身的一部分。具体可以从以下几个方面理解：
